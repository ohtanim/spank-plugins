...
#include "spank_qrmi_version.h" /* generated by CMake (configure_file) */

/*
 * Build/version marker for spank_qrmi itself, visible via
 * `strings spank_qrmi.so` or `readelf -p .version_info spank_qrmi.so`
 * without running the plugin. QRMI embeds its own version/git-hash into
 * the same `.version_info` section via its own crate, so it doesn't need
